...
const raw = config.sessionId.replace(/^CYBERMASTER-MD:~/, ''); // strip prefix if present
...
console.log(
chalk.green(
figlet.textSync('CYBERMASTER-MD', {
font: 'Standard',
...
console.log(chalk.cyan('🤖 CYBERMASTER-MD is starting up...'));
}
...
// Keep the group metadata cache warm whenever group info changes
...
const quotes = JSON.parse(fs.readFileSync(path.join(__dirname, 'config', 'autobioQuotes.json'), 'utf8'));
    ...
    const bioText = `CYBERMASTER-MD is alive now\n${dateStr} ${timeStr}\n"${quote}"`;
...
// Automatically clear in-memory caches every 6 hours
...
// Kick everything off
const startupDelay = parseInt(process.env.CYBERMASTER_RESTART_DELAY_MS || '0', 10);
setTimeout(async () => {
printBanner();
await fetchCore();
